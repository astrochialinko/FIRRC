...
def crossmatch(X1, X2, max_distance=np.inf):
    ...
    ---------------------
    :param           X1: array, first dataset, shape(N1, D1).
    :param           X2: array, second dataset, shape(N2, D2), D1 should be the same as D2.
    :param max_distance: float (optional), maximum radius of search.
                            If no point is within the given radius, then inf will be returned.
    ---------------------
    :return         ind: ndarrays, The index of the closest point in X2 to each point in X1
                         Array is length N1.
...
def crossmatch_angular(X1, X2, max_distance=np.inf, return_dist=False):
    ...
    ---------------------
    :param           X1: array, first dataset, shape(N1, 2).
                                X1[:, 0] is the RA in degrees.
                                X1[:, 1] is the DEC in degrees.
    :param           X2: array, second dataset, shape(N2, 2).
                                X2[:, 0] is the RA in degrees.
                                X2[:, 1] is the DEC in degrees.
    :param max_distance: float (optional), maximum radius of search, measured in degrees.
                                If no point is within the given radius, then inf will be returned.
    :param  return_dist: bool (optional),   If True, return the distance array,
                                otherwise only return the index array (default).
    ---------------------
    :return ind, (dist): ndarrays, the index and angular distance of each pair.
                         Locations with no match are indicated by
                         dist[i] = inf, ind[i] = []
    """
...
def merge_df(df1, df2, ind, dist=None):
    ...
    :param      ind: ndarrays, the index of each pair.
    :param     dist: ndarrays (optional), the angular distance (in degree) of each pair.
                        If given, then the new column 'Separation' (in arcsec) will be 
                        added to the returned merged dataset.
    ---------------------
    :return     mfd: DataFrame, the merge dataset
    ...
    if dist is not None:    
        ...
        mdf['Separation'] = sep
        mdf['Separation'].replace(np.inf, np.nan, inplace=True)
        mdf.sort_values(by=[mdf.columns[0], 'Separation'])
